...
import igc2kmz.pygooglechart as pygooglechart

from igc2kmz.color import bilinear_gradient, default_gradient
from igc2kmz.coord import rad_to_cardinal
import igc2kmz.kml as kml
import igc2kmz.kmz as kmz
from igc2kmz.scale import Scale, TimeScale, ZeroCenteredScale
import igc2kmz.util as util
...
class Stock(object):
    ...
    def __init__(self):
        ...
        pixel = open(os.path.join(BASE_DIR, self.pixel_url), 'rb').read()
        ...
        animation_icon = open(os.path.join(BASE_DIR, animation_icon_url), 'rb').read()
...
class Flight(object):
    ...
    def make_animation(self, globals):
        ...
        folder.add(placemark)
        for i in range(1, len(self.track.coords)):
